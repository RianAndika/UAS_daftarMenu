...
public class SharePerefernceUtility {


    private static final String PREF_FILE = "com.example.daftarmenu.DATA";
    ...
    */
     ...
     */
    public static List<Daftar> getAllMenu(Context ctx) {
        String jsonString = getSharedPref(ctx).getString(TRANS_KEY, null);
        List<Daftar> trs = new ArrayList<Daftar>();
        if (jsonString != null) {
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    trs.add(Daftar.fromJSONObject(obj));
                }
    ...
    /*
        ...
        Perhatikan bahwa data disimpan dalam format JSON String
     */
    private static void saveAllMenu(Context ctx, List<Daftar> trs) {
     ...
     */
    public static void addMenu(Context ctx, Daftar tr) {
        List<Daftar> trs = getAllMenu(ctx);
        trs.add(tr);
        saveAllMenu(ctx, trs);
    }
}
